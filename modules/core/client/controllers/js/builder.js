	...
	for(x=0; x<=99; x++) {
		...
		if( localStorage.getItem("blocksElement"+x) !== null && localStorage.getItem("blocksFrame"+x) !== null ) {
			...
			if( x >= 1 && pageNames[x] != undefined) {
				...
				newLI = $('<li><a class="plink" href="#'+pageNames[x]+'">'+pageNames[x]+'</a><span class="pageButtons"><a class="fileEdit" href=""><span class="fui-new"></span></a><a class="fileCopy" href=""><span class="fui-windows"></span></a><a class="fileDel" href=""><span class="fui-cross"></span></a><a href="#" class="btn btn-xs btn-primary btn-embossed fileSave"><span class="fui-check"></span></a></span></li>');
...
function addEventButtonModal(button, modal, wrapper, innerDoc) {
	$(button).on('click', function(e) {
		var closeBtn = $(this).parent().find('.close-inner-modal');
		$(innerDoc).find('.modal.fade.in').attr('style', 'display: none;');
		if (closeBtn[0] !== undefined) {
			$(closeBtn).remove();
		}
		$(modal).find('.modal-dialog').attr('style', 'margin-top: 60px; transform: none; -webkit-transform: none;');
		$(modal).attr('style', 'background-color: #333333; display: block; opacity: 1;');
		$(modal).addClass('in');
		$('.modal-backdrop').hide();
		var closeButton = $('<button type="button" class="btn btn-warning pull-right actionButtons close-inner-modal">close this popup</button>');
...
function showInnerModals(iFrame) {
	var innerDoc = iFrame.contentDocument || iFrame.contentWindow.document;
	var modalContactWindow = $(innerDoc).find('.modal.fade');
	if (modalContactWindow[0] !== undefined) {

		var parentLi = $(iFrame).parent();

		var ContactSection = $(parentLi).find('.wrapper-for-modal-button');
		if (ContactSection[0] === undefined) {

			var wrapper = $('<div class="wrapper-for-modal-button"></div>');
			...
			$(parentLi).on('mouseleave', function(){
				$(wrapper).attr('style', 'opacity:0;');
			});

			$(innerDoc).find('.modal.fade').on('click', function (e) {
				if (e.target.className !== 'modal-content' || e.srcElement.className !== 'modal-content') {
					var closeButton = $(parentLi).find('.close-inner-modal');
					var modal = $(innerDoc).find('.modal.fade.in');
			...
			$(modalContactWindow).each(function(index, element){
				if ($(element).attr('id') === 'modalContactSuccess') {
					var showSuccessButton = $('<button type="button" class="btn btn-primary pull-right actionButtons">Show "Success popup"</button>');
					$(wrapper).append(showSuccessButton);
					addEventButtonModal(showSuccessButton, element, wrapper, innerDoc);
				} else if ($(element).attr('id') === 'modalContactError') {
					var showErrorButton = $('<button type="button" class="btn btn-primary pull-right actionButtons">Show "Error popup"</button>');
					$(wrapper).append(showErrorButton);
					addEventButtonModal(showErrorButton, element, wrapper, innerDoc);
				} else {
					var openModalPopup = $('<button type="buton" class="btn btn-primary pull-right actionButtons">"Open modal popup"</button>');
					$(wrapper).append(openModalPopup);
					addEventButtonModal(openModalPopup, element, wrapper, innerDoc);
				}

			});

			$(parentLi).append(wrapper);
...
var dottedContainer = new Object();

function activeStyling() {

	/*this functional need for safari to remove border dotted when mouse move out element
	 start crutch */
	$('.substrate-header').unbind('mouseenter').unbind('mouseleave');

	$('.substrate-header').on('mouseenter', function() {
		removeDotted(dottedContainer);
	});

	function removeDotted(dottedContainer){

		if (Object.keys(dottedContainer)[0] !== undefined) {
			for (item in dottedContainer) {

				dottedContainer[item].css({'outline': '', 'cursor': '', 'outline-offset': ''});
				//dottedContainer[item].closest('section').css({'outline': '', 'cursor': '', 'outline-offset': ''});
			}
		}

	}

	$('.substrate-header').on('mouseleave', function () {
		if (Object.keys(dottedContainer)[0] !== undefined) {
			dottedContainer[item].css({'outline': '3px dotted red', 'cursor': 'pointer'});
			//dottedContainer[item].closest('section').css({'outline': '', 'cursor': '', 'outline-offset': ''});
		}
	});
		...
		for( var key in editableItems ) {
			...
			$(this).contents().find( pageContainer + ' '+ key ).hover( function(e){
				...
				if( $(this).closest('body').width() != $(this).width() ) {
					...
					dottedContainer[$(this)[0].dataset.selector] = $(this);
				...
				delete dottedContainer[$(this)[0].dataset.selector];
...
$(function(){
	...
	$('input:radio[name=mode]').on('toggle', function(){
		...
		if( $(this).val() == 'content' ) {
			...
			$('#pageList ul li iframe').each(function(){
				...
				var ContactSection = $(this).parent().find('.wrapper-for-modal-button');
				if (ContactSection[0] !== undefined) {
					$(ContactSection[0]).remove();
					$(this).parent().unbind('mouseenter mouseleave').unbind();
				}
			});
	...
	var editingIframeObj = new Object();
	...
	$('#pageList').on("click", ".frameCover > .htmlBlock", function(){
		...
		$('#modalHTMLEditor .editSaveButton')[0].dataset.iframeId = $(this).parent().parent().find('iframe').attr('id');
		editingIframeObj = $(this).parent().parent().find('iframe')[0];
		...
		var theEditor = $('#modalHTMLEditor .aceEditor');
		theEditor.uniqueId();

		//set the editor height
		theEditor.height('500') ;
	...
	$('#modalHTMLEditor').on("click", ".editSaveButton", function(){

		//enable draggable on the LI
		//$(this).closest('li').parent().sortable('enable');


		var theId = $('#modalHTMLEditor .aceEditor').attr('id');

		var theContent = aceEditors[theId].getValue();

		//var theiFrame = $('#' + $(this)[0].dataset.iframeId);
		var theiFrame = $(editingIframeObj);
		...
		var attr = $(theiFrame).attr('data-sandbox');
		...
		if (typeof attr !== typeof undefined && attr !== false) {
			...
			$(theiFrame).show().contents().find( pageContainer ).html( theContent );
		...
		//height adjustment
		//heightAdjustment( $(theiFrame).attr('id'), true );
		...
		var innerDoc = theiFrame[0].contentDocument || theiFrame[0].contentWindow.document;

		var eventNavElement = new CustomEvent('reload-script');
		innerDoc.dispatchEvent(eventNavElement);
	...
	$('#exportModal').on('shown.bs.modal', function (e) {
		...
		var imagesInProject = new Array();

		var videoInProject = new Array();
		...
		$('#pageList > ul').each(function(){
			...
			var regImg = /[^\/]+(\.jpg|\.png)+/gi;


			var images = newInput.val().match(regImg);

			if (images !== null) {
				imagesInProject = imagesInProject.concat(images);
			}

			var regVideo = /[^\/]+(\.mp4|\.ogv)+/gi;

			var video = newInput.val().match(regVideo);

			if (video !== null) {
				videoInProject = videoInProject.concat(video);
			}
		});

		function appendMedia(inProject, nameInput) {
			var i = inProject.length;
			inProject.sort();
			while (i--) {
				if (inProject[i] == inProject[i-1]) {
					inProject.splice(i, 1);
				}
			}

			var item = $('<input type="hidden" name="' + nameInput + '" value="">');

			item.val(inProject.join('||'));

			$('#exportModal form').prepend( item );
		}

		if (imagesInProject[0] !== undefined) {
			appendMedia(imagesInProject, 'images');
		}

		if (videoInProject[0] !== undefined) {
			appendMedia(videoInProject, 'video');
		}
	...
	$('#pages').on('click', 'li.active .fileCopy', function(){
		...
		var newPageLI = $('#pages li:eq('+theLIIndex+')').clone();

		var newInput = $('<input type="text" value="page' + $("#pages li").size() + '" name="page">');

		if ($(newPageLI).find('.fileDel')[0] === undefined) {
			var deleteButton = $('<a class="fileDel" href="#"><span class="fui-cross"></span></a>');
			newPageLI.find('a.fileCopy').after(deleteButton);
		}

		newPageLI.find('a:first').remove();

		newPageLI.prepend( newInput );
		...
		//create the page structure
		theULIndex = parseInt(theLIIndex)-1;
		newPageList = $('#pageList > ul:eq('+theULIndex+')').clone();
		newPageList.css('display','block');
		newPageList.attr('id', 'page'+($('#pages li').size()-1) );
